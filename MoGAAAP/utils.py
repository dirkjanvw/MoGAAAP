...
def find_base_dir():
    ...
    if os.path.exists(os.path.join(source_dir, "workflow")) and os.path.exists(
    ...
    if os.path.exists(os.path.join(module_dir, "workflow")) and os.path.exists(
    ...
    for path in sys.path:
        if "site-packages" in path or "dist-packages" in path:
            if os.path.exists(os.path.join(path, "workflow")) and os.path.exists(
    ...
    raise FileNotFoundError(
        "Could not find the base directory of the script or package."
    )
...
def init_mogaaap(workdir):
    ...
    click.secho(
        "        before running the pipeline by hand or using `MoGAAAP configure`",
        fg="blue",
    )
    click.secho(f"[INFO ] See {workdir}/config/example.yaml", fg="blue")
    click.secho("        for an example configuration", fg="blue")
    click.secho(
        f"[INFO ] Optionally, download the databases using `MoGAAAP download_databases`",
        fg="blue",
    )
...
def download_databases(workdir, databases):
    ...
    if "all" in databases or "kraken2" in databases:
        ...
        if os.path.exists(kraken2_dir):
            click.secho(
                "[WARN ] kraken2 database already exists; skipping download",
                fg="yellow",
            )
        else:
            click.secho(
                f"[INFO ] Downloading kraken2 database to {kraken2_dir}", fg="blue"
            )
            os.makedirs(kraken2_dir)

            kraken2_download_cmd = [
                "wget",
                "https://genome-idx.s3.amazonaws.com/kraken/k2_core_nt_20241228.tar.gz",
                "-O",
                os.path.join(kraken2_dir, "k2_core_nt_20241228.tar.gz"),
            ]
            ...
            click.secho(
                f"[INFO ] kraken2 database downloaded to {kraken2_dir}", fg="blue"
            )
    ...
    if "all" in databases or "oma" in databases:
        ...
        if os.path.exists(oma_dir):
            click.secho(
                "[WARN ] OMA database already exists; skipping download", fg="yellow"
            )
        else:
            ...
            oma_download_cmd = [
                ...
                "-O",
                os.path.join(oma_dir, "LUCA.h5"),
            ]
            if not run_command(oma_download_cmd):
                return

            click.secho(
                f"[INFO ] OMA database downloaded to {oma_dir}/LUCA.h5", fg="blue"
            )
    ...
    if "all" in databases or "gxdb" in databases:
        ...
        if os.path.exists(gxdb_dir):
            click.secho(
                "[WARN ] GXDB database already exists; skipping download", fg="yellow"
            )
        else:
            ...
            gxdb_helper_cmd = [
                ...
                "-O",
                os.path.join(gxdb_dir, "s5cmd_2.0.0_Linux-64bit.tar.gz"),
            ]
...
def configure_mogaaap(
    ...
    if os.path.exists(configfile):
        ...
        while os.path.exists(
            os.path.join(workdir, "config", f"config_{old_config_counter}.yaml")
        ):
            old_config_counter += 1
        old_config_location = os.path.join(
            workdir, "config", f"config_{old_config_counter}.yaml"
        )

        click.secho(
            f"[WARN ] Configuration file already exists; moving it to {old_config_location}",
            fg="yellow",
        )
    ...
    with open(samples) as f:
        ...
        for column in required_columns:
            if column not in header:
                click.secho(
                    f'[ERROR] Required column "{column}" not found in {samples}',
                    fg="red",
                )
                return
        ...
        if required_oneof_column_count == 0:
            click.secho(
                f"[ERROR] One of the required columns {', '.join(required_oneof_columns)} not found in {samples}",
                fg="red",
            )
            return
        for column in header:
            if (
                ...
                and column not in required_oneof_columns
                and column not in optional_columns
            ):
                click.secho(
                    f'[WARN ] Unknown column "{column}" found in {samples}', fg="yellow"
                )
    ...
    if reference_name not in reference_names:
        click.secho(
            f'[WARN ] Reference genome "{reference_name}" not found in {samples}; only found {", ".join(reference_names)}',
            fg="yellow",
        )
    if len(reference_names) > 1:
        click.secho(
            f"[WARN ] Multiple reference genomes found in {samples}: {', '.join(reference_names)}",
            fg="yellow",
        )
        click.secho(
            f'[WARN ] Please manually add them to the {configfile} file like "{reference_name}"',
            fg="yellow",
        )
    ...
    kmer_size = int(
        log(reference_length * (1 - collision_rate) / collision_rate) / log(4)
    )
    ...
    click.secho(
        f"        file with the example in {os.path.join(workdir, 'config', 'example.yaml')}",
        fg="blue",
    )
    click.secho("        for more information on the configuration options", fg="blue")
    click.secho(
        "[INFO ] After finishing the configuration, run the pipeline using `MoGAAAP run`",
        fg="blue",
    )
...
def run_mogaaap(workdir, configfile, reportfile, cores, memory, dryrun, other, targets):
    ...
    if not os.path.exists(configfile):
        if not os.path.exists(os.path.join(workdir, "config", "config.yaml")):
            click.secho("[ERROR] Configuration file does not exist", fg="red")
            click.secho(
                f"[ERROR] Checked {os.path.abspath(configfile)} and {os.path.abspath(os.path.join(workdir, 'config', 'config.yaml'))}",
                fg="red",
            )
            click.secho(
                "[ERROR] Did you run `MoGAAAP init` and `MoGAAAP configure`?", fg="red"
            )
            return
        else:
    ...
    fix_report_cmd = ["sed", "-E", "s/([^l]) h-screen/\1/g", reportfile + ".tmp.html"]
    ...
    click.secho(
        f"[INFO ] Final output available at {os.path.join(workdir, 'final_output')}",
        fg="blue",
    )
