...
def build_arg_parser():
 ...
 the terminal
    ...
    parser.add_argument('-i', '--input', type=str,
                        help="Folder with OMArk results.", required=True)
    parser.add_argument('-o', '--output', type=str,
                        help="Path to the figure to be outputted (PNG format).",
                        default="./omark_multiple.png")
    parser.add_argument('-m', '--mapping', type=str,
                        help="Path to a mapping file. Mapping file shoud be in the tsv format and have a header that lists 'Filename', 'Species name' and 'TaxId'. Filename should indicate the prefix of the sum file. Refer to the template for example.",
                        default=None)
    parser.add_argument('-t', '--taxonomy',
                        help="Flag whether the data should be ranked by taxonomy order. Only works if the TaxId is provided in the mapping file.",
                        action='store_true')
        ...
        for line in omaqsum.readlines():
            if not in_cont:
                protein_nr_line = re.search(
                    "On the whole proteome, there \w+ ([0-9]+) proteins", line)
                if protein_nr_line:
                ...
                if resultline:
                    ...
                    main_data['Expected_Duplicated'] = float(
                        resultline.group(3))
                    main_data['Unexpected_Duplicated'] = float(
                        resultline.group(4))
                ...
                conservline = re.search(
                    "A:([-0-9.]+)%\[P:([-0-9.]+)%,F:([-0-9.]+)%\],I:([-0-9.]+)%\[P:([-0-9.]+)%,F:([-0-9.]+)%\],C:([-0-9.]+)%\[P:([-0-9.]+)%,F:([-0-9.]+)%\],U:([-0-9.]+)%",
                    line)
                if conservline:
                    main_data['Consistent'] = float(conservline.group(1))
                    main_data['Consistent_Partially_Mapping'] = float(
                        conservline.group(2))
                    main_data['Consistent_Fragments'] = float(
                        conservline.group(3))
                    main_data['Consistent_Structurally_Consistent'] = float(
                        conservline.group(1)) - float(
                        conservline.group(2)) - float(conservline.group(3))
                    main_data['Inconsistent'] = float(conservline.group(4))
                    main_data['Inconsistent_Partially_Mapping'] = float(
                        conservline.group(5))
                    main_data['Inconsistent_Fragments'] = float(
                        conservline.group(6))
                    main_data['Inconsistent_Structurally_Consistent'] = float(
                        conservline.group(4)) - float(
                        conservline.group(5)) - float(conservline.group(6))
                    main_data['Contaminant'] = float(conservline.group(7))
                    main_data['Contaminant_Partially_Mapping'] = float(
                        conservline.group(8))
                    main_data['Contaminant_Fragments'] = float(
                        conservline.group(9))
                    main_data['Contaminant_Structurally_Consistent'] = float(
                        conservline.group(7)) - float(
                        conservline.group(8)) - float(conservline.group(9))
        ...
        if len(detected_species) > 1:
            for sup_detect_species in detected_species[1:]:
                contaminant_data.append(
                    {'Filename': filebase, 'Species_name': filebase,
...
def create_df_from_results(file_list):
    ...
    omark_df = pd.DataFrame(all_main_data,
                            columns=['Protein_number', 'Filename',
                                     'Species name', 'Complete', 'Single',
                                     'Duplicated', 'Expected_Duplicated',
                                     'Unexpected_Duplicated', 'Missing',
                                     'Consistent',
                                     'Consistent_Partially_Mapping',
                                     'Consistent_Fragments',
                                     'Consistent_Structurally_Consistent',
                                     'Inconsistent',
                                     'Inconsistent_Partially_Mapping',
                                     'Inconsistent_Fragments',
                                     'Inconsistent_Structurally_Consistent',
                                     'Contaminant',
                                     'Contaminant_Partially_Mapping',
                                     'Contaminant_Fragments',
                                     'Contaminant_Structurally_Consistent',
                                     'Unknown'])
    cont_df = pd.DataFrame(all_cont_data,
                           columns=['Filename', 'Species_name', 'Main_Taxon',
                                    'Contaminant', 'Contaminant_Taxid',
                                    'Number_of_Proteins'])
...
def integrate_external_data(omark_df, cont_df, mapping_file,
                            taxonomy_order=False):
    mapping = read_mapping_file(mapping_file)
    omark_df['Species name'] = [mapping.get(x, {}).get('Species name', x) for x
                                in omark_df['Filename']]
    omark_df['Taxid'] = [mapping.get(x, {}).get('Taxid', None) for x in
                         omark_df['Filename']]
    omark_df['Taxonomy'] = [mapping.get(x, {}).get('Taxonomy', None) for x in
                            omark_df['Filename']]
    if taxonomy_order:
        omark_df = omark_df.sort_values(by='Taxonomy')
    cont_df['Species name'] = [mapping.get(x, {}).get('Species name', x) for x
                               in cont_df['Filename']]
        ...
        for line in f.readlines():
            ...
            for i, elem in enumerate(cat):
                ...
                if datalabel[i].lower() == 'filename':
                    ...
                    taxid = elem
                    try:
                        taxonomy = ncbi.translate_to_names(
...
def plot_omark_df(omark_df, savefile=None, width=None, height=None,
                  no_labels=False, fontsize=12):
    if width == None:
        width = max(len(omark_df) * 0.6, 6)
    if height == None:
        height = 10
    fig, axes = plt.subplots(3, 1, figsize=(width, height))
    omark_df.plot.bar(y='Protein_number', width=1.0, ax=axes[0], lw=0.2,
                      edgecolor='whitesmoke', )

    omark_df.plot.bar(y=['Single', 'Duplicated', 'Missing'],
                      label=['Single', 'Duplicated', 'Missing'],
                      color=['#46bea1ff', '#cfee8eff', '#ed1c5aff'],
                      ylim=(0, 100), width=1.0, stacked=True, ax=axes[1],
                      rasterized=True)
    xticks = axes[0].get_xaxis().set_visible(False)

    xticks = axes[1].get_xaxis().set_visible(False)
    omark_df.plot.bar(
        y=['Consistent_Structurally_Consistent', 'Consistent_Partially_Mapping',
           'Consistent_Fragments',
           'Contaminant_Structurally_Consistent',
           'Contaminant_Partially_Mapping', 'Contaminant_Fragments',
           'Inconsistent_Structurally_Consistent',
           'Inconsistent_Partially_Mapping', 'Inconsistent_Fragments',
           'Unknown'], x='Species name',
        label=['Consistent', '', '', 'Contamination', '', '', 'Inconsistent',
               '', '', 'Unknown'],
        color=['#3db7e9ff', '#3db7e9ff', '#3db7e9ff', '#e69f00ff', '#e69f00ff',
               '#e69f00ff', '#8a5df1ff', '#8a5df1ff', '#8a5df1ff', '#000000ff'],
        edgecolor='white', lw=0.2, ylim=(0, 100), width=1.0, stacked=True,
        ax=axes[2], rasterized=True)
    xticks = axes[2].xaxis.get_major_ticks()
    bars = axes[2].patches
    hatches = ['', '///', '///', '', '///', '///', '', '///', '///', '']
    col = ['white', 'darkslategray', 'whitesmoke', 'white', 'darkslategray',
           'whitesmoke', 'white', 'darkslategray', 'whitesmoke', 'white']
    for index, bar in enumerate(bars):
        bar.set_hatch(hatches[index // len(omark_df)])
        bar.set_edgecolor(col[index // len(omark_df)])

    partial_patch = mpatches.Patch(facecolor='white', lw=0.1, hatch='///',
                                   edgecolor='darkslategray',
                                   label='Partial mapping', alpha=.99)
    fragment_patch = mpatches.Patch(facecolor='darkslategray', lw=0.1,
                                    hatch='///', edgecolor='white',
                                    label='Fragments', alpha=.99)
    custom_legend = [partial_patch, fragment_patch]
    axes[2].legend(title='Consistency',
                   handles=axes[2].get_legend_handles_labels()[
                               0] + custom_legend, loc='center left',
                   bbox_to_anchor=(1, 0.5), fontsize=fontsize)
    axes[1].legend(title='Completeness', loc='center left',
                   bbox_to_anchor=(1, 0.5), fontsize=fontsize)
...
if __name__ == '__main__':
    ...
    folders = [os.path.join(input_folder, x) for x in os.listdir(input_folder)
               if os.path.isdir(os.path.join(input_folder, x))]
    main_df, cont_df = create_df_from_results(folders)
    if mapping_file:
        main_df, cont_df = integrate_external_data(main_df, cont_df,
                                                   mapping_file,
                                                   taxonomy_order=arg.taxonomy)
    if len(main_df) > 0:
