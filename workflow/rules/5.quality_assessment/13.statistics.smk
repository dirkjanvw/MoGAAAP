rule individual_statistics:
    input:
        ...
        qv = expand("results/{{asmname}}/5.quality_assessment/01.merqury/{k}/hifi/{{asmname}}_vs_hifi.qv", k=config["k_qa"]),
        kmerstats = expand("results/{{asmname}}/5.quality_assessment/01.merqury/{k}/hifi/{{asmname}}_vs_hifi.completeness.stats", k=config["k_qa"]),
    ...
    shell:
        """
        (
        printf "Name\\tTotal length\\t#sequences\\tN50\\t#genes (full)\\t#genes (coding)\\t#transcripts (coding)\\t#chromosomes\\tTotal length (chromosomes)\\t#unassigned sequences\\tTotal length (unassigned sequences)\\tTotal QV (HiFi)\\tK-mer completeness (HiFi)\\t#contigs\\tContig N50\\tInput data\\tAssembler\\n" > {output.tsv}
