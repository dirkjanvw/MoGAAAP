rule create_renaming_table:
    ...
    log:
        "results/logs/2.scaffolding/renaming_sequences/{asmname}.vs.{reference}.min{minlen}.k{k}.w{w}.n1.all.scaffolds.log"
    benchmark:
        "results/benchmarks/2.scaffolding/renaming_sequences/{asmname}.vs.{reference}.min{minlen}.k{k}.w{w}.n1.all.scaffolds.txt"
    params:
    ...
    shell: # I found the following awk commands to get a quick list of what chromosomes belong together (based on https://github.com/bcgsc/ntJoin/issues/63):
        ...
        awk 'BEGIN{{OFS = "'\''";}} {{print $1,$6;}}' {input.agp} | \
            ...
            uniq -c | \
            sort -nr | \
            awk -vnum_chr="{params.num_chr}" 'FNR<=num_chr{{print $2,$3;}}' > {output}
        ) &> {log}
        """

rule renaming_sequences:
    input:
        expand("results/{{asmname}}/2.scaffolding/03.renaming/{{asmname}}.vs.{reference}.min{minlen}.k{k}.w{w}.n1.all.scaffolds.conversion.tsv",
